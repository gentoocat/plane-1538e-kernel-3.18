...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	//mid_czy add for ausw /spk pa
	[GPIO_SPKSD_HIGH] = {"spksd-pullhigh", false, NULL},
	[GPIO_SPKSD_LOW] = {"spksd-pulllow", false, NULL},
	[GPIO_AUSW_HIGH] = {"ausw-pullhigh", false, NULL},
	[GPIO_AUSW_LOW] = {"ausw-pulllow", false, NULL},
	//add end
};
...
//mid_czy add for ausw /spk ap
int AudDrv_GPIO_SPKSD_Select(int bEnable)
{
	int retval = 0;

	if (bEnable == 1) {
		if (aud_gpios[GPIO_SPKSD_HIGH].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_SPKSD_HIGH].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_SPKSD_HIGH] pins\n");
		}
	} else {
		if (aud_gpios[GPIO_SPKSD_LOW].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_SPKSD_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_SPKSD_LOW] pins\n");
		}
...
int AudDrv_GPIO_AUSW_Select(int bEnable)
{
	int retval = 0;

	if (bEnable == 1) {
		if (aud_gpios[GPIO_AUSW_HIGH].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_AUSW_HIGH].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_AUSW_HIGH] pins\n");
		}
	} else {
		if (aud_gpios[GPIO_AUSW_LOW].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_AUSW_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_AUSW_LOW] pins\n");
		}
