...
#ifdef CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT
		DBG(0, "xuchen begin usb_in:%d,  vdc:%d, vusb:%d\n",
				usb_in,DISO_data.diso_state.cur_vdc_state, DISO_data.diso_state.cur_vusb_state);
       if ((DISO_data.diso_state.cur_vusb_state==DISO_OFFLINE)&&usb_in)
	   	usb_in = false;
		DBG(0, "xuchen end usb_in:%d,  vdc:%d, vusb:%d\n",
				usb_in,DISO_data.diso_state.cur_vdc_state, DISO_data.diso_state.cur_vusb_state);
#endif
