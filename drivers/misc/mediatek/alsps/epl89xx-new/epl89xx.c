...
*******************************************************************************/
#if ANDR_M
#define COMPATIABLE_NAME "mediatek,epl89xx"
...
#define LUX_PER_COUNT			400//700		// 0.7  // 输出值= 原始值 *LUX_PER_COUNT/1000, 光感上报的数值
#define GES_ALS_COUNT           200     //if gesture is enabled, als report rate

/*ALS interrupt table*/
unsigned long epl89xx_als_lux_intr_level[] = {15, 39, 63, 316, 639, 4008, 5748, 10772, 14517, 65535};
unsigned long epl89xx_als_adc_intr_level[10] = {0};

int epl89xx_als_frame_time = 0;
int epl89xx_ps_frame_time = 0;
 ...
 *  configuration
 ******************************************************************************/
struct hwmsen_object *epl89xx_ps_hw, * epl89xx_als_hw;
static struct epl89xx_priv *epl_sensor_obj = NULL;
#if !MTK_LTE
static struct platform_driver epl89xx_alsps_driver;
#endif
static struct wake_lock ps_lock;
static struct mutex sensor_mutex;
struct input_dev epl89xx_dev;
...
#else
#define APS_FUN(f)              	  	pr_debug(KERN_INFO APS_TAG"%s\n", __FUNCTION__)
#define APS_ERR(fmt, args...)    	    pr_debug(KERN_ERR  APS_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define APS_LOG(fmt, args...)    	    pr_debug(KERN_INFO APS_TAG fmt, ##args)
#define APS_DBG(fmt, args...)    	    pr_debug(KERN_INFO fmt, ##args)
#endif

static int epl89xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int epl89xx_i2c_remove(struct i2c_client *client);
static int epl89xx_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
static int epl89xx_i2c_suspend(struct i2c_client *client, pm_message_t msg);
static int epl89xx_i2c_resume(struct i2c_client *client);
#if ANDR_M
static irqreturn_t epl89xx_eint_func(int irq, void *desc);
#else   /*ANDR_M*/
static void epl89xx_eint_func(void);
#endif  /*ANDR_M*/
...
void epl89xx_restart_polling(void);
void epl89xx_fast_update(struct i2c_client *client);
void epl89xx_update_mode(struct i2c_client *client);
int epl89xx_read_ps(struct i2c_client *client);
int epl89xx_read_als(struct i2c_client *client);
static int als_sensing_time(int intt, int adc, int cycle);
static int ps_sensing_time(int intt, int adc, int cycle);
static int epl89xx_als_intr_update_table(struct epl89xx_priv *epld);
int epl89xx_factory_ps_data(void);
int epl89xx_factory_als_data(void);
#if !MTK_LTE
int epl89xx_ps_operate(void* self, uint32_t command, void* buff_in, int size_in, void* buff_out, int size_out, int* actualout);
int epl89xx_als_operate(void* self, uint32_t command, void* buff_in, int size_in, void* buff_out, int size_out, int* actualout);
#endif
#if 1  // tongjun@mid @{
...
#if PS_DYN_K
void epl89xx_restart_dynk_polling(void);
#endif
...
/*----------------------------------------------------------------------------*/
struct epl89xx_i2c_addr      /*define a series of i2c slave address*/
{
    u8  write_addr;
};
...
{
    .probe     	= epl89xx_i2c_probe,
    .remove     = epl89xx_i2c_remove,
    .detect     = epl89xx_i2c_detect,
    .suspend    = epl89xx_i2c_suspend,
    .resume     = epl89xx_i2c_resume,
...
static struct alsps_init_info epl89xx_init_info = {
		.name = EPL_DEV_NAME,
...
int epl89xx_dynk_min_ps_raw_data = 0xffff;
int epl89xx_dynk_max_ir_data;

u32 epl89xx_dynk_thd_low = 0;
u32 epl89xx_dynk_thd_high = 0;

int epl89xx_dynk_low_offset;
int epl89xx_dynk_high_offset;
#endif
...
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_500, EPL_ALS_INTT_500};
static int als_dynamic_intt_value[] = {500, 500};
...
//====================I2C write operation===============//
*/
static int epl89xx_I2C_Write_Block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
...
static int epl89xx_I2C_Write_Cmd(struct i2c_client *client, uint8_t regaddr, uint8_t data, uint8_t txbyte)
{
    uint8_t buffer[2];
    int ret = 0;
    int retry;
...
static int epl89xx_I2C_Write(struct i2c_client *client, uint8_t regaddr, uint8_t data)
{
    int ret = 0;
    ret = epl89xx_I2C_Write_Cmd(client, regaddr, data, 0x02);
    return ret;
}

static int epl89xx_I2C_Read(struct i2c_client *client, uint8_t regaddr, uint8_t bytecount)
{
    int ret = 0;
    int retry;
    ...
    while(bytecount>0)
    {
        epl89xx_I2C_Write_Cmd(client, regaddr+read_count, 0x00, 0x01);

        for(retry = 0; retry < I2C_RETRY_COUNT; retry++)
        {
            rx_count = bytecount>epl89xx_i2c_max_count?epl89xx_i2c_max_count:bytecount;
...
#if 1 // tongjun@mid @{
void epl_notify_event(int event)
{
...
static int epl89xx_als_intr_update_table(struct epl89xx_priv *epld)
{
	int i;
	for (i = 0; i < 10; i++) {
		if ( epl89xx_als_lux_intr_level[i] < 0xFFFF )
		{
#if ALS_DYN_INTT
		    if(epl_sensor.als.report_type == CMC_BIT_DYN_INT)
		    {
                epl89xx_als_adc_intr_level[i] = epl89xx_als_lux_intr_level[i] * 1000 / c_gain;
		    }
		    else
#endif
		    {
                epl89xx_als_adc_intr_level[i] = epl89xx_als_lux_intr_level[i] * 1000 / epl_sensor.als.factory.lux_per_count;
		    }


            if(i != 0 && epl89xx_als_adc_intr_level[i] <= epl89xx_als_adc_intr_level[i-1])
    		{
                epl89xx_als_adc_intr_level[i] = 0xffff;
    		}
		}
		else {
			epl89xx_als_adc_intr_level[i] = epl89xx_als_lux_intr_level[i];
		}
		APS_LOG(" [%s]:epl89xx_als_adc_intr_level: [%d], %ld \r\n", __func__, i, epl89xx_als_adc_intr_level[i]);
	}
...
static void write_global_variable(struct i2c_client *client)
{
    u8 buf;
#if HS_ENABLE
    struct epl89xx_priv *obj = epl_sensor_obj;
    bool enable_hs = test_bit(CMC_BIT_HS, &obj->enable) && atomic_read(&obj->hs_suspend)==0;
#endif
    epl89xx_I2C_Write(client, 0x11, EPL_POWER_OFF | EPL_RESETN_RESET);
    //wake up chip
    //buf = epl_sensor.reset | epl_sensor.power;
    //epl89xx_I2C_Write(client, 0x11, buf);

    /* read revno*/
    epl89xx_I2C_Read(client, 0x20, 2);
    epl_sensor.revno = gRawData.raw_bytes[0] | gRawData.raw_bytes[1] << 8;
    /*osc setting*/
    epl89xx_I2C_Write(client, 0xfc, epl_sensor.osc_sel| EPL_GFIN_ENABLE | EPL_VOS_ENABLE | EPL_DOC_ON);
...
#endif
    {
        ...
        epl89xx_I2C_Write(client, 0x05, buf);
        buf = epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type;
        epl89xx_I2C_Write(client, 0x06, buf);
        buf = epl_sensor.ps.compare_reset | epl_sensor.ps.lock;
        epl89xx_I2C_Write(client, 0x1b, buf);
        epl89xx_I2C_Write(client, 0x22, (u8)(epl_sensor.ps.cancelation& 0xff));
        epl89xx_I2C_Write(client, 0x23, (u8)((epl_sensor.ps.cancelation & 0xff00) >> 8));
        ...
        epl89xx_I2C_Write(client, 0x01, buf);
        buf = epl_sensor.als.adc | epl_sensor.als.cycle;
        epl89xx_I2C_Write(client, 0x02, buf);
        buf = epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type;
        epl89xx_I2C_Write(client, 0x07, buf);
        buf = epl_sensor.als.compare_reset | epl_sensor.als.lock;
        epl89xx_I2C_Write(client, 0x12, buf);
        ...
        epl89xx_I2C_Write(client, 0x24, buf);
        ...
        epl89xx_I2C_Write(client, 0x25, buf);
    ...
    epl89xx_I2C_Write(client, 0x00, buf);

}

static int write_factory_calibration(struct epl89xx_priv *epl_data, char* ps_data, int ps_cal_len)
{
    struct file *fp_cal;
...
static bool read_factory_calibration(void)
{
    ...
    if(epl_sensor.ps.factory.calibration_enable && !epl_sensor.ps.factory.calibrated)
    {
        ...
        epl89xx_I2C_Write(obj->client,0x22, (u8)(epl_sensor.ps.cancelation& 0xff));
        epl89xx_I2C_Write(obj->client,0x23, (u8)((epl_sensor.ps.cancelation & 0xff00) >> 8));
        set_psensor_intr_threshold(epl_sensor.ps.low_threshold, epl_sensor.ps.high_threshold);
...
static int epl89xx_run_ps_calibration(struct epl89xx_priv *epl_data)
{
    struct epl89xx_priv *epld = epl_data;
    u16 ch1=0;
...
static void initial_global_variable(struct i2c_client *client, struct epl89xx_priv *obj)
{
    //general setting
    ...
    epl_sensor.als.integration_time = EPL_ALS_INTT_500;  // 积分时间， 长的话，数据大些
    ...
    epl_sensor.als.report_type = CMC_BIT_RAW; // 原始数据
    ...
    if(epl_sensor.als.polling_mode == 0)
        epl89xx_als_intr_update_table(obj);
    ...
    epl_sensor.ps.integration_time = EPL_PS_INTT_225 ; // EPL_PS_INTT_35;    // 2. 和光感一样， 距离远则加大，   EPL_PS_INTT_225
    epl_sensor.ps.gain = EPL_GAIN_MID ; // EPL_GAIN_LOW;      //   3. 大的话，距离远， 容易受到背景和光线的干扰， 最后调整这个, 尽量不改 EPL_GAIN_MID
    ...
    epl_sensor.ps.ir_boost = EPL_IR_BOOST_100;   // 1. 可调为200,但最好vcc和VLED分开供电 为2倍， 也可增加感应距离
...
#if PS_DYN_K
    epl89xx_dynk_max_ir_data = 50000; // ps max ch0
    epl89xx_dynk_low_offset = 150; //  1000;   // 远离  150
    epl89xx_dynk_high_offset = 300; // 1600;  // 接近  300
#endif
    //ps factory
    epl_sensor.ps.factory.calibration_enable =  false;
...
#if HS_ENABLE
	//hs setting
    epl_sensor.hs.integration_time = EPL_PS_INTT_55;
	epl_sensor.hs.integration_time_max = EPL_PS_INTT_225;
	epl_sensor.hs.integration_time_min = EPL_PS_INTT_28;
...
static int epl89xx_get_als_value(struct epl89xx_priv *obj, u16 als)
{
    int idx;
    int invalid = 0;
    int level=0;
    ...
    switch(epl_sensor.als.report_type)
    {
		...
		case CMC_BIT_DYN_INT:
            ...
            if(change_flag == true)
            {
                ...
                dynamic_intt_high_thr = als_dynamic_intt_high_thr[dynamic_intt_idx];
                dynamic_intt_low_thr = als_dynamic_intt_low_thr[dynamic_intt_idx];
                epl89xx_fast_update(obj->client);

                //epl89xx_I2C_Write(obj->client, 0x11, EPL_POWER_OFF | EPL_RESETN_RESET);    // setting with epl_sensor_fast_update()
                //epl89xx_I2C_Write(obj->client, 0x01, epl_sensor.als.integration_time | epl_sensor.als.gain); //setting with epl_sensor_fast_update()
                mutex_lock(&sensor_mutex);
                epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.wait | epl_sensor.mode);
                epl89xx_I2C_Write(obj->client, 0x11, EPL_POWER_ON | EPL_RESETN_RUN);
            ...
            if(epl_sensor.als.polling_mode == 0)
            {
    		    ...
    		    {
        			if (lux <= (*(epl89xx_als_lux_intr_level + idx))) {
        				level = idx;
        				if (*(epl89xx_als_lux_intr_level + idx))
        					break;
        			}
        			if (idx == 9) {
        				level = idx;
...
static int set_psensor_intr_threshold(uint16_t low_thd, uint16_t high_thd)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
    ...
    mutex_lock(&sensor_mutex);
    epl89xx_I2C_Write_Block(epld->client, 0x0c, buf, 4);
    mutex_unlock(&sensor_mutex);
...
static int set_lsensor_intr_threshold(uint16_t low_thd, uint16_t high_thd)
{
    ...
    buf[0] = low_lsb  = (uint8_t) (low_thd & 0x00ff);
    mutex_lock(&sensor_mutex);
    epl89xx_I2C_Write_Block(epld->client, 0x08, buf, 4);
    mutex_unlock(&sensor_mutex);
    APS_LOG("%s: low_thd = %d, high_thd = %d \n", __FUNCTION__, low_thd, high_thd);
    return 0;
}

static void epl89xx_dumpReg(struct i2c_client *client)
{
...
int hw8k_init_device_epl89xx(struct i2c_client *client)
{
    APS_LOG("hw8k_init_device_epl89xx.........\r\n");
...
static void epl89xx_intr_als_report_lux(void)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    u16 als;
    int err = 0;
    ...
    APS_LOG("[%s]: IDEL MODE \r\n", __func__);
    epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.wait | EPL_MODE_IDLE);

    als = epl89xx_get_als_value(obj, epl_sensor.als.data.channels[1]);

    mutex_lock(&sensor_mutex);
    epl89xx_I2C_Write(obj->client, 0x12, EPL_CMP_RESET | EPL_UN_LOCK);
    epl89xx_I2C_Write(obj->client, 0x11, EPL_POWER_OFF | EPL_RESETN_RESET);  //After runing CMP_RESET, dont clean interrupt_flag
    mutex_unlock(&sensor_mutex);
        ...
        if(epl_sensor.als.report_type == CMC_BIT_DYN_INT)
        {
            if(dynamic_intt_idx == 0)
            {
                ...
                if((als == 0)) //Note: als =0 is nothing
                    ...
                    low_thd = (*(epl89xx_als_adc_intr_level + (als-1)) + 1) * normal_value;
                high_thd = (*(epl89xx_als_adc_intr_level + als)) * normal_value;
            ...
            {
                epl_sensor.als.low_threshold = *(epl89xx_als_adc_intr_level + (als-1)) + 1;
                epl_sensor.als.high_threshold = *(epl89xx_als_adc_intr_level + als);
...
static void epl89xx_report_ps_status(void)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
#if MTK_LTE
    int ps_state;
    ...
    if(err != 0)
    {
        APS_ERR("epl89xx_eint_work err: %d\n", err);
    	epl89xx_I2C_Write(epld->client,0x1b, EPL_CMP_RESET | EPL_UN_LOCK);
    }
#else
...
int epl89xx_get_addr(struct alsps_hw *hw, struct epl89xx_i2c_addr *addr)
{
    if(!hw || !addr)
    {
        return -EFAULT;
...
static void epl89xx_power(struct alsps_hw *hw, unsigned int on)
{
...
static void epl89xx_gesture_rawdata(void)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    //APS_FUN();
    mutex_lock(&sensor_mutex);
    epl89xx_I2C_Read(obj->client, 0x27, 6*index);
    ...
    for(i=0; i<index; i+=shift)
    {
        ...
        for(k = 0 ; k<width; k++)
        {
            ...
            if(i+k>=index || k==width-1)
            {
                ...
                add_gesture_data(gRawData.ges_data);   // tongjun@mid
...
void epl_sleep_change(int is_sleep)
{
    ...
    if(is_sleep)
    {
        ...
        epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait_sleep);
        epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait_sleep| epl_sensor.mode);
    }
    else
    {
        epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait);
	epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait | epl_sensor.mode);
    }
    APS_LOG("sleep mode: %d\n", sleep_mode);
}
#endif // @}
/*----------------------------------------------------------------------------*/
...
int epl89xx_read_als(struct i2c_client *client)
{
    struct epl89xx_priv *obj = i2c_get_clientdata(client);
    u8 buf[5];
    if(client == NULL)
    {
    ...
    mutex_lock(&sensor_mutex);
    epl89xx_I2C_Read(obj->client, 0x12, 5);
...
int epl89xx_read_ps(struct i2c_client *client)
{
    ...
    mutex_lock(&sensor_mutex);
    epl89xx_I2C_Read(client,0x1b, 5);
    buf[0] = gRawData.raw_bytes[0];
    buf[1] = gRawData.raw_bytes[1];
    buf[2] = gRawData.raw_bytes[2];
    buf[3] = gRawData.raw_bytes[3];
...
#if HS_ENABLE
int epl89xx_read_hs(struct i2c_client *client)
{
    u8 buf;
    ...
    epl89xx_I2C_Read(client,0x1e, 2);
...
int epl89xx_factory_als_data(void)
{
    ...
    if(enable_als == 1)
    {
        epl89xx_read_als(epld->client);

        if(epl_sensor.als.report_type == CMC_BIT_DYN_INT)
        {
            int als_lux=0;
            als_lux = epl89xx_get_als_value(epld, epl_sensor.als.data.channels[1]);
        }
    }
    else
    {
...
void epl89xx_enable_ps(int enable)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
    ...
    if(enable_ps != enable)
    {
        if(enable)
        {
            ...
            epl89xx_dynk_min_ps_raw_data = 0xffff;
        ...
        epl89xx_fast_update(epld->client); //if fast to on/off ALS, it read als for DOC_OFF setting for als_operate
        epl89xx_update_mode(epld->client);
    }
    else
    {
        APS_LOG("[%s]: ps_enable_flag is the same \r\n", __func__);
    }
}

void epl89xx_enable_als(int enable)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
...
#endif
        epl89xx_fast_update(epld->client);
        }
        else
        {
    ...
    {
        APS_LOG("[%s]: als_enable_flag is the same \r\n", __func__);
    }
}
...
void epl89xx_polling_work(struct work_struct *work)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
...
#endif
    {
        epl89xx_gesture_rawdata();

        if(epl_sensor.ges.wait == EPL_WAIT_SINGLE)
        {
            epl89xx_I2C_Write(obj->client, 0x11,  epl_sensor.power | epl_sensor.reset);
        }
...
void epl89xx_dynk_thd_polling_work(struct work_struct *work)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    if(enable_ps == true)
    {
        epl89xx_read_ps(obj->client);

        if( (epl89xx_dynk_min_ps_raw_data > epl_sensor.ps.data.data)
            && (epl_sensor.ps.saturation == 0)
            && (epl_sensor.ps.data.ir_data < epl89xx_dynk_max_ir_data) )
        {
            epl89xx_dynk_min_ps_raw_data = epl_sensor.ps.data.data;
            epl89xx_dynk_thd_low = epl89xx_dynk_min_ps_raw_data + epl89xx_dynk_low_offset;
		    epl89xx_dynk_thd_high = epl89xx_dynk_min_ps_raw_data + epl89xx_dynk_high_offset;

            if(epl89xx_dynk_thd_low>65534)
                epl89xx_dynk_thd_low = 65534;
            if(epl89xx_dynk_thd_high>65535)
                epl89xx_dynk_thd_high = 65535;

		    APS_LOG("[%s]:dyn ps raw = %d, min = %d, ir_data = %d\n", __func__, epl_sensor.ps.data.data, epl89xx_dynk_min_ps_raw_data, epl_sensor.ps.data.ir_data);
		    set_psensor_intr_threshold((u16)epl89xx_dynk_thd_low, (u16)epl89xx_dynk_thd_high);
		    APS_LOG("[%s]:dyn k thre_l = %d, thre_h = %d\n", __func__, (u16)epl89xx_dynk_thd_low, (u16)epl89xx_dynk_thd_high);
        }
...
void epl89xx_fast_update(struct i2c_client *client)
{
    ...
    epl89xx_I2C_Write(client, 0x02, epl_sensor.als.adc | EPL_CYCLE_1);
    epl89xx_I2C_Write(client, 0x01, epl_sensor.als.integration_time | epl_sensor.als.gain);

    epl89xx_I2C_Write(client, 0x00, epl_sensor.wait | EPL_MODE_ALS);
    epl89xx_I2C_Write(client, 0x11, EPL_POWER_ON | EPL_RESETN_RUN);
    mutex_unlock(&sensor_mutex);
    ...
    if(epl_sensor.als.polling_mode == 0)
    {
        ...
        epl89xx_I2C_Write(client, 0x00, epl_sensor.wait | EPL_MODE_IDLE);
        epl89xx_I2C_Write(client, 0x12, EPL_CMP_RESET | EPL_UN_LOCK);
        epl89xx_I2C_Write(client, 0x12, EPL_CMP_RUN | EPL_UN_LOCK);
    }

    epl89xx_I2C_Write(client, 0x11, EPL_POWER_OFF | EPL_RESETN_RESET);

    epl89xx_I2C_Write(client, 0x02, epl_sensor.als.adc | epl_sensor.als.cycle);
...
void epl89xx_update_mode(struct i2c_client *client)
{
    ...
    epl89xx_als_frame_time = als_time;
    epl89xx_ps_frame_time = ps_time;
...
static void epl89xx_eint_work(struct work_struct *work)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    bool enable_ges = test_bit(CMC_BIT_GES, &obj->enable) && atomic_read(&obj->ges_suspend)==0;
    bool enable_ps = test_bit(CMC_BIT_PS, &obj->enable) && atomic_read(&obj->ps_suspend)==0;
    ...
    else
    {
        ...
        if(epl_sensor.ps.interrupt_flag == EPL_INT_TRIGGER)
        {
            if(enable_ps)
            {
                ...
                epl89xx_report_ps_status();
            }
            //PS unlock interrupt pin and restart chip
            mutex_lock(&sensor_mutex);
    		epl89xx_I2C_Write(obj->client,0x1b, EPL_CMP_RUN | EPL_UN_LOCK);
    		mutex_unlock(&sensor_mutex);
        }

        if(epl_sensor.als.interrupt_flag == EPL_INT_TRIGGER)
        {
            epl89xx_intr_als_report_lux();
            //ALS unlock interrupt pin and restart chip
        	mutex_lock(&sensor_mutex);
        	epl89xx_I2C_Write(obj->client,0x12, EPL_CMP_RUN | EPL_UN_LOCK);
        	mutex_unlock(&sensor_mutex);
        }
    }
...
int epl89xx_setup_eint(struct i2c_client *client)
{
	...
	if (obj->irq_node)
	{
		...
		if (request_irq(obj->irq, epl89xx_eint_func, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
    ...
    mt65xx_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, epl89xx_eint_func, 0);
    mt65xx_eint_unmask(CUST_EINT_ALS_NUM);
#else

	mt_eint_set_hw_debounce(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_TYPE, epl89xx_eint_func, 0);
...
static int epl89xx_init_client(struct i2c_client *client)
{
#if !ANDR_M
    struct epl89xx_priv *obj = i2c_get_clientdata(client);
#endif
    ...
    if(obj->hw->polling_mode_ps == 0)
    {
        ...
        mt_eint_unmask(CUST_EINT_ALS_NUM);
#endif
        if((err = epl89xx_setup_eint(client)))
        {
...
#endif /*ANDR_M*/
    if((err = hw8k_init_device_epl89xx(client)) != 0)
    {
        APS_ERR("init epl89xx_dev: %d\n", err);
        return err;
    }
    return err;
}
...
/*----------------------------------------------------------------------------*/
static ssize_t epl89xx_show_reg(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
...
static ssize_t epl89xx_show_status(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    struct epl89xx_priv *epld = epl_sensor_obj;
...
#if PS_DYN_K
        len += snprintf(buf+len, PAGE_SIZE-len, "Dyn thr(L/H) = (%d/%d)\n", (u16)epl89xx_dynk_thd_low, (u16)epl89xx_dynk_thd_high);
#endif
...
static ssize_t epl89xx_store_als_enable(struct device_driver *ddri, const char *buf, size_t count)
{
    uint16_t mode=0;
    APS_FUN();
    ...
    epl89xx_enable_als(mode);

    return count;
}

static ssize_t epl89xx_store_ps_enable(struct device_driver *ddri, const char *buf, size_t count)
{
    uint16_t mode=0;
    APS_FUN();
    ...
    APS_LOG("[%s]: ps enable=%d \r\n", __func__, mode);
    epl89xx_enable_ps(mode);

    return count;
}

static ssize_t epl89xx_show_cal_raw(struct device_driver *ddri, char *buf)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    u16 ch1=0;
    ssize_t len = 0;
...
/*----------------------------------------------------------------------------*/
static ssize_t epl89xx_store_threshold(struct device_driver *ddri,const char *buf, size_t count)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    int low, high;
    APS_FUN();
...
static ssize_t epl89xx_show_threshold(struct device_driver *ddri, char *buf)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    ssize_t len = 0;
...
static ssize_t epl89xx_store_ges_enable(struct device_driver *ddri, const char *buf, size_t count)
{
    int mode=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
	...
	// set_gesture_mode(mode);   // tongjun@mid 
    epl89xx_update_mode(client);

    return count;
}

static ssize_t epl89xx_show_ges_raws(struct device_driver *ddri, char *buf)
{
    u16 *tmp = (u16*)buf;
...
static ssize_t epl89xx_store_ges_int_time(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    int val;
    ...
    epl89xx_I2C_Write(obj->client, 0x24, epl_sensor.ges.adc | epl_sensor.ges.integration_time |epl_sensor.ges.gain);
    epl89xx_I2C_Write(obj->client, 0x00, epl_sensor.wait | epl_sensor.mode);
    return count;
}

static ssize_t epl89xx_store_wait_time(struct device_driver *ddri, const char *buf, size_t count)
{
    int val;
...
static ssize_t epl89xx_store_ges_work_threshold(struct device_driver *ddri, const char *buf, size_t count)
{	
	int val = 0; 
    sscanf(buf, "%d",&val);
	WORK_TH = ( unsigned short)val;
    crosstalk_offset=WORK_TH/3;
    return count;
}

static ssize_t epl89xx_store_ges_polling_mode(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    int value;
    u8 w_buf;
    ...
    w_buf = epl_sensor.ges.interrupt_enable | epl_sensor.ges.interrupt_index | epl_sensor.ges.ir_mode | epl_sensor.ges.ir_boost | epl_sensor.ges.ir_drive;
    epl89xx_I2C_Write(obj->client, 0x25, w_buf);

    epl89xx_update_mode(obj->client);

    return count;
}

static ssize_t epl89xx_store_gain(struct device_driver *ddri, const char *buf, size_t count)
{
	struct epl89xx_priv *epld = epl_sensor_obj;
	int value = 0;
    APS_FUN();
	...
	switch (epl_sensor.mode)
	{
        case EPL_MODE_PS:
	        ...
	        epl89xx_I2C_Write(epld->client, 0x03, epl_sensor.ps.integration_time | epl_sensor.ps.gain);
		break;

        case EPL_MODE_ALS: //als
            epl_sensor.als.gain = value;
	        epl89xx_I2C_Write(epld->client, 0x01, epl_sensor.als.integration_time | epl_sensor.als.gain);
		break;

		case EPL_MODE_GES: //ges
            epl_sensor.ges.gain = value;
	        epl89xx_I2C_Write(epld->client, 0x24, epl_sensor.ges.integration_time | epl_sensor.ges.gain);
		break;
    }
...
static ssize_t epl89xx_store_ges_zoom_delta(struct device_driver *ddri, const char *buf, size_t count)
{
    sscanf(buf, "%d", &zoom_delta);
    return count;
}

static ssize_t epl89xx_store_mode(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
    int value=0;
    APS_FUN();
...
static ssize_t epl89xx_store_ir_mode(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            ...
            epl89xx_I2C_Write(obj->client, 0x05, epl_sensor.ps.ir_on_control | epl_sensor.ps.ir_mode | epl_sensor.ps.ir_boost | epl_sensor.ps.ir_drive);
            break;
        ...
        case EPL_MODE_GES_SS: //ps
            ...
            epl89xx_I2C_Write(obj->client, 0x25, epl_sensor.ges.interrupt_enable | epl_sensor.ges.interrupt_index | epl_sensor.ges.ir_mode | epl_sensor.ges.ir_boost | epl_sensor.ges.ir_drive);
            break;
    }
...
static ssize_t epl89xx_store_ir_contrl(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    uint8_t  data;
    struct epl89xx_priv *obj = epl_sensor_obj;
...
static ssize_t epl89xx_store_ir_boost(struct device_driver *ddri, const char *buf, size_t count)
{
    int data;
    struct epl89xx_priv *obj = epl_sensor_obj;
...
static ssize_t epl89xx_store_ir_drive(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
...
static ssize_t epl89xx_store_interrupt_type(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            if(!obj->hw->polling_mode_ps)
            {
                ...
                epl89xx_I2C_Write(obj->client, 0x06, epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type);
        ...
        case EPL_MODE_ALS: //als
            if(!obj->hw->polling_mode_als)
            {
                ...
                epl89xx_I2C_Write(obj->client, 0x07, epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type);
...
static ssize_t epl89xx_store_ges_index(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    uint8_t  data;
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    APS_LOG("%s: 0x25 = 0x%x\n", __FUNCTION__, data);
    epl89xx_I2C_Write(obj->client, 0x25, data);
    epl89xx_update_mode(obj->client);

    return count;
}

static ssize_t epl89xx_store_ps_offset(struct device_driver *ddri, const char *buf, size_t count)
{
    int data = 0;
    struct epl89xx_priv *obj = epl_sensor_obj;
    APS_FUN();
    ...
    epl89xx_I2C_Write(obj->client, 0x22, (u8)(data & 0xff));
    epl89xx_I2C_Write(obj->client, 0x23, (u8)((data & 0xff00) >> 8));

    epl89xx_I2C_Read(obj->client, 0x23, 2);
...
static ssize_t epl89xx_store_osc(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    uint8_t  data;
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    APS_LOG("%s: 0xfc = 0x%x \n", __FUNCTION__, data);
    epl89xx_I2C_Write(obj->client, 0xfc, data);
    epl89xx_update_mode(obj->client);

    return count;
}

static ssize_t epl89xx_store_integration(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            ...
            epl89xx_I2C_Write(obj->client, 0x03, epl_sensor.ps.integration_time | epl_sensor.ps.gain);
            epl89xx_I2C_Read(obj->client, 0x03, 1);
        ...
        case EPL_MODE_ALS: //als
            ...
            epl89xx_I2C_Write(obj->client, 0x01, epl_sensor.als.integration_time | epl_sensor.als.gain);
            epl89xx_I2C_Read(obj->client, 0x01, 1);
        ...
        case EPL_MODE_GES:
            ...
            epl89xx_I2C_Write(obj->client, 0x24, epl_sensor.ges.adc | epl_sensor.ges.integration_time |epl_sensor.ges.gain);
            epl89xx_I2C_Read(obj->client, 0x24, 1);
...
static ssize_t epl89xx_store_adc(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            ...
            epl89xx_I2C_Write(obj->client, 0x04, epl_sensor.ps.adc | epl_sensor.ps.cycle);
            epl89xx_I2C_Read(obj->client, 0x04, 1);
        ...
        case EPL_MODE_ALS: //als
            ...
            epl89xx_I2C_Write(obj->client, 0x02, epl_sensor.als.adc | epl_sensor.als.cycle);
            epl89xx_I2C_Read(obj->client, 0x02, 1);
...
static ssize_t epl89xx_store_cycle(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
...
static ssize_t epl89xx_store_als_report_type(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
    int value=0;
    APS_FUN();
    ...
    if(epl_sensor.als.polling_mode == 0)
        epl89xx_als_intr_update_table(epld);
    return count;
}

static ssize_t epl89xx_store_ps_polling_mode(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
#if !MTK_LTE
    ...
    obj_ps.sensor_operate = epl89xx_ps_operate;
    ...
    epl89xx_I2C_Write(epld->client, 0x06, epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type);
    epl89xx_I2C_Write(epld->client, 0x07, epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type);
...
static ssize_t epl89xx_store_als_polling_mode(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
#if !MTK_LTE
    ...
    obj_als.sensor_operate = epl89xx_als_operate;
...
static ssize_t epl89xx_store_ps_w_calfile(struct device_driver *ddri, const char *buf, size_t count)
{
	struct epl89xx_priv *epld = epl_sensor_obj;
...
static ssize_t epl89xx_store_unlock(struct device_driver *ddri, const char *buf, size_t count)
{
	struct epl89xx_priv *epld = epl_sensor_obj;
    int mode;
    APS_FUN();
	...
	switch (mode)
	{
		case 0: //PS unlock and run
        	epl_sensor.ps.compare_reset = EPL_CMP_RUN;
        	epl_sensor.ps.lock = EPL_UN_LOCK;
        	epl89xx_I2C_Write(epld->client,0x1b, epl_sensor.ps.compare_reset |epl_sensor.ps.lock);
		break;
		...
		case 2: //ALS unlock and run
        	epl_sensor.als.compare_reset = EPL_CMP_RUN;
        	epl_sensor.als.lock = EPL_UN_LOCK;
        	epl89xx_I2C_Write(epld->client,0x12, epl_sensor.als.compare_reset | epl_sensor.als.lock);
		break;
...
static ssize_t epl89xx_store_als_ch_sel(struct device_driver *ddri, const char *buf, size_t count)
{
	struct epl89xx_priv *epld = epl_sensor_obj;
    int ch_sel;
    APS_FUN();
...
static ssize_t epl89xx_store_als_lux_per_count(struct device_driver *ddri, const char *buf, size_t count)
{
    sscanf(buf, "%d",&epl_sensor.als.factory.lux_per_count);
    return count;
}

static ssize_t epl89xx_store_i2c_max_count(struct device_driver *ddri, const char *buf, size_t count)
{
    sscanf(buf, "%d",&epl89xx_i2c_max_count);
    return count;
}

static ssize_t epl89xx_show_ps_polling(struct device_driver *ddri, char *buf)
{
    u16 *tmp = (u16*)buf;
...
static ssize_t epl89xx_show_als_polling(struct device_driver *ddri, char *buf)
{
    u16 *tmp = (u16*)buf;
...
static ssize_t epl89xx_show_ps_run_cali(struct device_driver *ddri, char *buf)
{
	struct epl89xx_priv *epld = epl_sensor_obj;
	ssize_t len = 0;
    int ret;
    ...
    ret = epl89xx_run_ps_calibration(epld);
...
static ssize_t epl89xx_show_pdata(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    int ps_raw;

    ps_raw = epl89xx_factory_ps_data();
    APS_LOG("[%s]: ps_raw=%d \r\n", __func__, ps_raw);
...
static ssize_t epl89xx_show_als_data(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    u16 als_raw = 0;

    als_raw = epl89xx_factory_als_data();
...
#if PS_DYN_K
static ssize_t epl89xx_store_dyn_offset(struct device_driver *ddri, const char *buf, size_t count)
{
    int dyn_h,dyn_l;
    APS_FUN();
    ...
    epl89xx_dynk_low_offset = dyn_l;
    epl89xx_dynk_high_offset = dyn_h;

    return count;
}

static ssize_t epl89xx_store_dyn_max_ir_data(struct device_driver *ddri, const char *buf, size_t count)
{
    int max_ir_data;
    APS_FUN();

    sscanf(buf, "%d",&max_ir_data);
    epl89xx_dynk_max_ir_data = max_ir_data;
...
#if ALS_DYN_INTT
static ssize_t epl89xx_store_c_gain(struct device_driver *ddri, const char *buf, size_t count)
{
    int als_c_gain;
    APS_FUN();
...
static ssize_t epl89xx_store_reg_write(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *epld = epl_sensor_obj;
    int reg;
    int data;
    ...
    epl89xx_I2C_Write(epld->client, reg, data);

    return count;
}

static ssize_t epl89xx_show_renvo(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
...
#if HS_ENABLE
static ssize_t epl89xx_store_hs_enable(struct device_driver *ddri, const char *buf, size_t count)
{
    uint16_t mode=0;
    struct epl89xx_priv *obj = epl_sensor_obj;
	bool enable_als = test_bit(CMC_BIT_ALS, &obj->enable) && atomic_read(&obj->als_suspend)==0;
...
static ssize_t epl89xx_store_hs_int_time(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl89xx_priv *obj = epl_sensor_obj;
	int value;
	u8 intt_buf;
	...
	epl89xx_I2C_Write(obj->client, 0x03, intt_buf);
	mutex_unlock(&hs_sensor_mutex);

    return count;
}

static ssize_t epl89xx_show_hs_raws(struct device_driver *ddri, char *buf)
{
    u16 *tmp = (u16*)buf;
...
static ssize_t epl89xx_show_als_intr_table(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    int idx = 0;
    ...
    for (idx = 0; idx < 10; idx++)
    {
        len += snprintf(buf+len, PAGE_SIZE-len, "[%d]: %ld", idx, (*(epl89xx_als_lux_intr_level + idx)));
	}

    len += snprintf(buf+len, PAGE_SIZE-len, "\n ALS ADC Table\n");
    for (idx = 0; idx < 10; idx++)
    {
        len += snprintf(buf+len, PAGE_SIZE-len, "[%d]: %ld", idx, (*(epl89xx_als_adc_intr_level + idx)));
	}
...
/*CTS --> S_IWUSR | S_IRUGO*/
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(elan_status,					S_IWUSR | S_IRUGO, epl89xx_show_status,  	  		    NULL										);
static DRIVER_ATTR(elan_reg,    				S_IWUSR | S_IRUGO, epl89xx_show_reg,   				NULL										);
static DRIVER_ATTR(als_enable,					S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_als_enable					);
static DRIVER_ATTR(als_report_type,				S_IWUSR | S_IRUGO, NULL,								epl89xx_store_als_report_type				);
static DRIVER_ATTR(als_polling_mode,			S_IWUSR | S_IRUGO, epl89xx_show_als_polling,   		epl89xx_store_als_polling_mode				);
static DRIVER_ATTR(als_lux_per_count,			S_IWUSR | S_IRUGO, NULL,   					 		epl89xx_store_als_lux_per_count				);
static DRIVER_ATTR(ps_enable,					S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_ps_enable					    );
static DRIVER_ATTR(ps_offset,					S_IWUSR | S_IRUGO, NULL,								epl89xx_store_ps_offset						);
static DRIVER_ATTR(ps_polling_mode,			    S_IWUSR | S_IRUGO, epl89xx_show_ps_polling,   			epl89xx_store_ps_polling_mode				);
static DRIVER_ATTR(ges_raws, 					S_IWUSR | S_IRUGO, epl89xx_show_ges_raws, 	  		    NULL										);
static DRIVER_ATTR(ges_enable,					S_IWUSR | S_IRUGO, NULL,   					 		epl89xx_store_ges_enable					);
static DRIVER_ATTR(ges_int_time,				S_IWUSR | S_IRUGO, NULL,   					 		epl89xx_store_ges_int_time					);
static DRIVER_ATTR(ges_polling_mode,			S_IWUSR | S_IRUGO, NULL,   					 		epl89xx_store_ges_polling_mode				);
static DRIVER_ATTR(ges_work_threshold,			S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_ges_work_threshold			);
static DRIVER_ATTR(ges_zoom_delta,				S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_ges_zoom_delta				);
static DRIVER_ATTR(ges_index,					S_IWUSR | S_IRUGO, NULL,								epl89xx_store_ges_index					    );
static DRIVER_ATTR(ir_mode,					    S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_ir_mode						);
static DRIVER_ATTR(ir_boost,					S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_ir_boost						);
static DRIVER_ATTR(ir_drive,					S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_ir_drive						);
static DRIVER_ATTR(ir_on,						S_IWUSR | S_IRUGO, NULL,								epl89xx_store_ir_contrl						);
static DRIVER_ATTR(interrupt_type,				S_IWUSR | S_IRUGO, NULL,								epl89xx_store_interrupt_type				);
static DRIVER_ATTR(osc,						    S_IWUSR | S_IRUGO, NULL,								epl89xx_store_osc							);
static DRIVER_ATTR(integration,					S_IWUSR | S_IRUGO, NULL,								epl89xx_store_integration					);
static DRIVER_ATTR(adc,						    S_IWUSR | S_IRUGO, NULL,								epl89xx_store_adc							);
static DRIVER_ATTR(cycle,						S_IWUSR | S_IRUGO, NULL,								epl89xx_store_cycle						    );
static DRIVER_ATTR(i2c_max_count,		        S_IWUSR | S_IRUGO, NULL,   					        epl89xx_store_i2c_max_count				    );
static DRIVER_ATTR(mode,						S_IWUSR | S_IRUGO, NULL,   							epl89xx_store_mode							);
static DRIVER_ATTR(wait_time,					S_IWUSR | S_IRUGO, NULL,   					 		epl89xx_store_wait_time						);
static DRIVER_ATTR(set_threshold,     			S_IWUSR | S_IRUGO, epl89xx_show_threshold,             epl89xx_store_threshold						);
static DRIVER_ATTR(cal_raw, 					S_IWUSR | S_IRUGO, epl89xx_show_cal_raw, 	  		    NULL										);
static DRIVER_ATTR(unlock,				        S_IWUSR | S_IRUGO, NULL,			                    epl89xx_store_unlock						);
static DRIVER_ATTR(als_ch,				        S_IWUSR | S_IRUGO, NULL,			                    epl89xx_store_als_ch_sel					);
static DRIVER_ATTR(run_ps_cali, 				S_IWUSR | S_IRUGO, epl89xx_show_ps_run_cali, 	  		NULL										);
static DRIVER_ATTR(pdata,                       S_IWUSR | S_IRUGO, epl89xx_show_pdata,                 NULL                                        );
static DRIVER_ATTR(als_data,                    S_IWUSR | S_IRUGO, epl89xx_show_als_data,              NULL                                        );
static DRIVER_ATTR(gain,				        S_IWUSR | S_IRUGO, NULL,			                    epl89xx_store_gain					        );
static DRIVER_ATTR(ps_w_calfile,				S_IWUSR | S_IRUGO, NULL,			                    epl89xx_store_ps_w_calfile				    );
static DRIVER_ATTR(i2c_w,                       S_IWUSR | S_IRUGO, NULL,                               epl89xx_store_reg_write                     );
static DRIVER_ATTR(elan_renvo,                  S_IWUSR | S_IRUGO, epl89xx_show_renvo,                 NULL                                        );
static DRIVER_ATTR(als_intr_table,				S_IWUSR | S_IRUGO, epl89xx_show_als_intr_table,        NULL                                        );
#if PS_DYN_K
static DRIVER_ATTR(dyn_offset,                  S_IWUSR | S_IRUGO, NULL,                               epl89xx_store_dyn_offset                    );
static DRIVER_ATTR(dyn_max_ir_data,             S_IWUSR | S_IRUGO, NULL,                               epl89xx_store_dyn_max_ir_data               );
#endif
#if ALS_DYN_INTT
static DRIVER_ATTR(als_dyn_c_gain,              S_IWUSR | S_IRUGO, NULL,                               epl89xx_store_c_gain                        );
#endif
#if HS_ENABLE
static DRIVER_ATTR(hs_enable,					S_IWUSR | S_IRUGO, NULL,                               epl89xx_store_hs_enable                     );
static DRIVER_ATTR(hs_int_time,					S_IWUSR | S_IRUGO, NULL,                               epl89xx_store_hs_int_time                   );
static DRIVER_ATTR(hs_raws,					    S_IWUSR | S_IRUGO, epl89xx_show_hs_raws,               NULL                                        );
#endif
...
static int epl89xx_create_attr(struct device_driver *driver)
{
    int idx, err = 0;
...
static int epl89xx_delete_attr(struct device_driver *driver)
{
    int idx ,err = 0;
    int num = (int)(sizeof(epl_sensor_attr_list)/sizeof(epl_sensor_attr_list[0]));
...
******************************************************************************/
static int epl89xx_open(struct inode *inode, struct file *file)
{
...
static int epl89xx_release(struct inode *inode, struct file *file)
{
    APS_FUN();
...
static long epl89xx_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    clear_bit(CMC_BIT_GES, &obj->enable);   // if factory mode, disable ges
    ...
    switch (cmd)
    {
        ...
        case ALSPS_GET_ALS_DATA:
            als_data = epl89xx_factory_als_data();
            dat = epl89xx_get_als_value(obj, als_data);
...
static struct file_operations epl_sensor_fops =
{
    .owner = THIS_MODULE,
    .open = epl89xx_open,
    .release = epl89xx_release,
    .unlocked_ioctl = epl89xx_unlocked_ioctl,
};
...
#if defined(CONFIG_HAS_EARLYSUSPEND)
static void epl89xx_early_suspend(struct early_suspend *h)
{
    /*early_suspend is only applied for ALS*/
    struct epl89xx_priv *obj = container_of(h, struct epl89xx_priv, early_drv);
    bool enable_ps = test_bit(CMC_BIT_PS, &obj->enable) && atomic_read(&obj->ps_suspend)==0;
    bool enable_als = test_bit(CMC_BIT_ALS, &obj->enable) && atomic_read(&obj->als_suspend)==0;
...
static void epl89xx_late_resume(struct early_suspend *h)
{
...
static int als_get_data(int* value, int* status)
{
    ...
    report_lux = epl89xx_get_als_value(obj, epl_sensor.als.data.channels[1]);
...
#else /*MTK_LTE*/
/*----------------------------------------------------------------------------*/
int epl89xx_ps_operate(void* self, uint32_t command, void* buff_in, int size_in,
                       void* buff_out, int size_out, int* actualout)
{
    ...
    hwm_sensor_data* sensor_data;
#if !PS_DYN_K
    struct epl89xx_priv *obj = (struct epl89xx_priv *)self;
#endif

    APS_LOG("epl89xx_ps_operate command = 0x%x\n",command);
    switch (command)
    {
        case SENSOR_DELAY:
            //msleep(5);
...
int epl89xx_als_operate(void* self, uint32_t command, void* buff_in, int size_in,
                        void* buff_out, int size_out, int* actualout)
{
    int err = 0;
    int value = 0;
    ...
    APS_FUN();
    APS_LOG("epl89xx_als_operate command = 0x%x\n", command);
...
static int epl89xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	client->addr = (0x82 >> 1);  // tongjun@mid Add 
    epl89xx_dumpReg(client);
...
#endif /*ANDR_M*/
obj->hw->i2c_addr[0] = 0x82;  // tongjun@mid Add 
    epl89xx_get_addr(obj->hw, &obj->addr);
    ...
    INIT_DELAYED_WORK(&obj->eint_work, epl89xx_eint_work);
    INIT_DELAYED_WORK(&obj->polling_work, epl89xx_polling_work);
#if PS_DYN_K
    INIT_DELAYED_WORK(&obj->dynk_thd_polling_work, epl89xx_dynk_thd_polling_work);
#endif
    ...
    epl_sensor_i2c_client = client;
    set_gesture_callback(epl_sleep_change, epl_notify_event);   // tongjun@mid 

    //initial global variable and write to senosr
    initial_global_variable(client, obj);

    if((err = epl89xx_init_client(client)))
    {
...
#if MTK_LTE /*MTK_LTE start .................*/
    if((err = epl89xx_create_attr(&epl89xx_init_info.platform_diver_addr->driver)))
    {
	...
	ps_ctl.is_polling_mode = epl_sensor.ps.polling_mode==1? true:false;  // tongjun@mid Add for fix for ps data report
#ifdef CUSTOM_KERNEL_SENSORHUB
    ...
    if((err = epl89xx_create_attr(&epl89xx_alsps_driver.driver)))
    {
        APS_ERR("create attribute err = %d\n", err);
        goto exit_create_attr_failed;
    }
    ...
    obj->early_drv.suspend  = epl89xx_early_suspend,
    obj->early_drv.resume   = epl89xx_late_resume,
    register_early_suspend(&obj->early_drv);
#endif

    if(obj->hw->polling_mode_ps == 0 || obj->hw->polling_mode_als == 0 || !epl_sensor.ges.polling_mode)
        epl89xx_setup_eint(client);
#if MTK_LTE
    alsps_init_flag = 0;
#endif
    APS_LOG("%s: OK\n", __FUNCTION__);
...
static int epl89xx_i2c_remove(struct i2c_client *client)
{
    int err;
#if MTK_LTE
    if((err = epl89xx_delete_attr(&epl89xx_init_info.platform_diver_addr->driver)))
    {
        APS_ERR("epl89xx_delete_attr fail: %d\n", err);
    }
#else
    if((err = epl89xx_delete_attr(&epl_sensor_i2c_driver.driver)))
    {
        APS_ERR("epl89xx_delete_attr fail: %d\n", err);
    }
#endif
...
#if !MTK_LTE
/*----------------------------------------------------------------------------*/
static int epl89xx_probe(struct platform_device *pdev)
{
#if !ANDR_M
...
/*----------------------------------------------------------------------------*/
static int epl89xx_remove(struct platform_device *pdev)
{
#if !ANDR_M
    struct alsps_hw *hw = get_cust_alsps_hw();
#endif /*ANDR_M*/
...
#endif
/*----------------------------------------------------------------------------*/
static int __init epl89xx_init(void)
{
#if ANDR_M
...
#if MTK_LTE
    alsps_driver_add(&epl89xx_init_info);
#else
    if(platform_driver_register(&epl89xx_alsps_driver))
    {
...
/*----------------------------------------------------------------------------*/
static void __exit epl89xx_exit(void)
{
    APS_FUN();
#if !MTK_LTE
    platform_driver_unregister(&epl89xx_alsps_driver);
#endif
}
/*----------------------------------------------------------------------------*/
module_init(epl89xx_init);
module_exit(epl89xx_exit);
/*----------------------------------------------------------------------------*/
