 ...
 * configuration
*******************************************************************************/
#define PSCTRL_VAL	 0x31  // 0x33	/* ps_persistance=4, ps_gain=64X, PS_IT=0.391ms */
...
static DEFINE_MUTEX(stk3x1x_mutex);
	...
	#define STK_LT_N_CT	 40 // 50  //
	#define STK_HT_N_CT	60 // 80 //100
...
//	obj->hw = get_cust_alsps_hw();
	...
	if((err = stk3x1x_write_sw_reset(client)))
	{
		APS_ERR("stk3x1x_i2c_probe i2c test error, err=%d", err);
		goto exit;
	}
...
//	APS_ERR("items sensor.alsps.i2 =%d\n",i2c_num);
	APS_ERR("hw->i2c_num =%d\n",hw->i2c_num);
