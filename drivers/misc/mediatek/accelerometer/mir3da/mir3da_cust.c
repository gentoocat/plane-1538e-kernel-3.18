...
static  int Log_level = 0x0;  // to turn on full log ,set value to 0xff
...
static int mir3da_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	MI_FUN;    
	// mid modify to fix when in sleep, mir3da_suspend cause system reboot problem  @{
	if(mir3da_init_flag == -1){
		return 0;
	}
	// @}
...
static const struct of_device_id accel_of_match[] = {
	#if 1	// kcm modify by phf, 20160614, gsensor 兼容
...
{    
	...
	// tongjun@mid @{
	direction = item_integer("accel.bma.orientation", 0);
    printk("%s accel.bma.orientation=%d\n", __func__, direction);
    direction = direction_calculate(direction);
    printk("%s mir3da direction=%d\n", __func__, direction);
