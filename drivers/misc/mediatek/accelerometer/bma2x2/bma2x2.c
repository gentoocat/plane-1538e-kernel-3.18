...
static int bma2x2_init_client(struct i2c_client *client, int reset_cali)
{
	...
	//res = bma2x2_set_fifo_mode(client, 0);
	//if (res != BMA2x2_SUCCESS)
	//	return res;
	//GSE_LOG("BMA2x2 fifo data cleared !\n");
...
/*----------------------------------------------------------------------------*/
int bma2x2_selftest(unsigned char *selfresult, unsigned char *diff4x, unsigned char *diff4y, unsigned char *diff4z)
{
	struct bma2x2_i2c_data *obj = obj_i2c_data;
...
static ssize_t show_selftest(struct device_driver *ddri, char *buf)
{
	...
	if (ret < 0)
		...
		if (selftestresult == 0)
		num =
			snprintf(buf, PAGE_SIZE, "selftest success, diff4x=%d, diff4y=%d, diff4z=%d\n", diff4x, diff4y, diff4z);
		else {
			X_failed = (selftestresult & 0x01) ? 1 : 0;
			Y_failed = (selftestresult & 0x02) ? 1 : 0;
			Z_failed = (selftestresult & 0x04) ? 1 : 0;
			num =
				snprintf(buf, PAGE_SIZE, "selftest failed, X_failed=%d, Y_failed=%d, Z_failed=%d, diff4x=%d, diff4y=%d, diff4z=%d\n", X_failed, Y_failed, Z_failed, diff4x, diff4y, diff4z);
		}
	}
	return num;
