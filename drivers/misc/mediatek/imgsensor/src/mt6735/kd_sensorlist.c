...
static inline int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	err = kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM *) g_invokeSocketIdx, g_invokeSensorNameStr,true, CAMERA_HW_DRVNAME1);
	/* wait for power stable */
       // tongjun@mid @{   for power on fail, do not search, otherwise it will cost a lot of time
       if(err != ERROR_NONE){   
