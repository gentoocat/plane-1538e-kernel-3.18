...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			LOG_INF("Read sensor id fail,i2c write id: 0x%x,sensor id :0x%x,imgsensor_info.sensor_id:0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		*sensor_id = 0xFFFFFFFF;
		LOG_INF("Read sensor id fail,----------sensor id :0x%x,imgsensor_info.sensor_id:0x%x\n",*sensor_id,imgsensor_info.sensor_id);
