...
{
	mutex_lock(&sensor_data_mutex);
	MAGN_ERR("ox=%d, oy=%d, oz=%d\n", sensor_data[13], sensor_data[14] , sensor_data[15]);
	*x = sensor_data[13] * CONVERT_O;
	*y = sensor_data[14] * CONVERT_O;
	*z = sensor_data[15] * CONVERT_O;
	*status = sensor_data[8];
       MAGN_ERR("x=%d, y=%d, z=%d\n", *x, *y, *z);
	mutex_unlock(&sensor_data_mutex);
	return 0;
}
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	MAGN_ERR("akm09911_i2c_probe, ADDR=%d\n", client->addr);
	data = kzalloc(sizeof(struct akm09911_i2c_data), GFP_KERNEL);
	if (!data) {
		err = -ENOMEM;
		goto exit;
	}
	client->addr = 0x0c;   // tongjun Add 
	data->hw = hw;
...
static int __init akm09911_init(void)
{
	...
	// tongjun@mid @{
	direction = item_integer("magnetometer.akm.orientation", 0);
//	if(direction >= 0 && direction <=8){
		hw->direction = direction;
//	}
	MAG_ERR("magnetometer.akm.orientation =%d\n", direction);
	// tongjun@mid @}	
