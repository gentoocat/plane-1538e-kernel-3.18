...
printk(KERN_ERR MSE_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
//static struct i2c_board_info __initdata bmm150_i2c_info
//= {I2C_BOARD_INFO(BMM150_DEV_NAME, BMM150_I2C_ADDR)};
...
static long bmm150_unlocked_ioctl(
...
static int __init bmm150_init(void)
{
	...
	if(!hw){
		MSE_ERR("mediatek,bmm150 get dts info fail\n");
	}
	// tongjun@mid @{
	direction = item_integer("msensor.bmm150.orientation", 0);
	if(direction >= 0 && direction <=7){
		hw->direction = direction;
	}
	MSE_ERR("msensor.bmm150.orientation =%d\n", direction);
	// tongjun@mid @}	 
//	i2c_register_board_info(hw->i2c_num, &bmm150_i2c_info, 1);  // tongjun 
