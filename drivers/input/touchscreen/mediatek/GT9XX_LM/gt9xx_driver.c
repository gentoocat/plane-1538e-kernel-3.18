...
static int item_send_cfg = 1; // 1 send config , 0 do not send config	
// tongjun@mid }
...
#if 0   // ifdef CONFIG_GTP_CUSTOM_CFG
...
#endif
//	client->addr = 0x14;     // tongjun@mid Modify 
...
void force_reset_guitar(void)
{
	...
	disable_irq(touch_irq); //gtp_irq_disable();
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
	...
	while (retry++ < 10) {
		...
		disable_irq(touch_irq); //	gtp_irq_disable();
			gtp_reset_guitar(client, 20);
			enable_irq(touch_irq); // gtp_irq_enable();
		} else {
#else
		{
#endif
...
{
	if((!item_exist("touchpanel.ic")) ||(!item_equal("touchpanel.ic", "gt9xx", 0)) ){
	...
	item_send_cfg = (item_integer("touch.gt9xx.send_cfg", 0)==0) ? 0: 1;   // tongjun@mid 
		return 0;
}
