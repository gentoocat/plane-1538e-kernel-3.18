...
// #endif
	...
	// to case 1. read TPD_RES_X from LCM, 2. change it by  CONFIG_MTK_LCM_PHYSICAL_ROTATION
#if 0  // mid Modify for replace MTK_LCM_PHYSICAL_ROTATION  with ro.sf.hwrotation @{
...
#else
	if(90 == hwrotation || 270 == hwrotation){
		TPD_RES_Y = DISP_GetScreenWidth();
		TPD_RES_X = DISP_GetScreenHeight();	
	}else{
		TPD_RES_X = DISP_GetScreenWidth();
...
// @}
	...
	|| defined(CONFIG_TOUCHSCREEN_MTK_SYNAPTICS_3320_50)
       ...
       // tongjun Add for pressure support @{
       if(item_integer("touchpanel.pressure_support",0) == 1){   
